...
This note explains how CheckMate’s AI features work: credentials and model selection from FIDO, the connection check, then the main flows (overview, follow-up, explain, propose fix, apply and validate, Fido-backed image reports + AI Image Sniff Test).

Implementation lives under `checkmate/ai/` (LiteLLM client, session, overview, explain, fix, context, resources). CheckMate does **not** import the FIDO Python package. Image reports are generated by an installed Fido app (`Fido.exe image-report`); chat uses FIDO’s on-disk credentials.
...
- `verapdf_flavour` — Tools → Settings… (`ua1` / `ua2` / PDF/A / WTPDF; default `ua2`)
...
## 6. Image reports via installed Fido
...
| Image report | N/A | — | Post-check **Images**; Fido `image-report` CLI + in-app HTML |
| AI Image Sniff Test | Fido CLI | — | Chosen up front (`--percent` / `--assess`); chat on the same page |
...
| Locate installed Fido | `checkmate/fido_launch.py` |
| Fido `image-report` CLI + JSON loader | `checkmate/ai/fido_image_report.py` |
| Image-report Q&A | `checkmate/ai/image_report_qa.py` |
| Verdict tally pills | `checkmate/ai/verdict_tally_bar.py` |
...
| Alt-text sniff / chat mixin | `checkmate/ai/alt_dialog.py` |
