...
class OverviewHtmlExportTests(unittest.TestCase):
    def test_includes_chat_when_requested(self):
        from checkmate.ai.markdown_html import (
            append_followup_markdown,
            overview_html_for_export,
        )

        md = append_followup_markdown(
            "Publication looks solid.",
            heading="Follow-up",
            question="Any missing alts?",
            answer="Yes, figure 3.",
        )
        out = overview_html_for_export(
            md, include_chat=True, title="AI overview"
        )
        self.assertIn("Publication looks solid", out)
        self.assertIn("Any missing alts?", out)
        self.assertIn("chat-bubble", out)
        self.assertIn("Generated by CheckMate", out)
        self.assertNotIn("checkmate://", out)

    def test_strips_chat_when_unchecked(self):
        from checkmate.ai.markdown_html import (
            append_followup_markdown,
            overview_html_for_export,
        )
        ...
        out = overview_html_for_export(
            md, include_chat=False, title="AI overview"
        )
        self.assertIn("Publication looks solid", out)
        self.assertNotIn("Any missing alts?", out)
        self.assertNotIn('class="chat-bubble chat-user"', out)
